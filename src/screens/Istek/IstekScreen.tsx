...
const IstekScreen: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([]);
  const [newMessage, setNewMessage] = useState('');

  useEffect(() => {
    const messagesRef = database().ref('/istekler/');
  ...
  const handleItemClick = (itemId: string) => {
    const messageRef = database().ref(`/istekler/${itemId}`);
  ...
  return (
    <View style={styles.screenContainer}>
      <View style={styles.headerContainer}>
        <Text style={styles.headerText}>İstekleriniz</Text>
      </View>
      <FlatList
        data={messages}
      ...
      <TextInput
          ...
          placeholder="İsteklerinizi buraya yazın..."
          multiline={true}
          numberOfLines={4}
